...
session_start();
	...
	if($_SERVER['REQUEST_METHOD'] == "POST")
	{
		...
		if(!empty($dane_imie) && !empty($dane_nazwisko) && !empty($dany_email) && !empty($dane_haslo) && strlen($dane_haslo) >7 && !is_numeric($dane_imie) && !is_numeric($dane_nazwisko) && (strpos($dany_email, "@") !== false) && (strpos($dany_email, ".") !== false))
		{
			...
			pg_query("INSERT INTO uzytkownicy(imie, nazwisko, email, haslo, rola) values ('$dane_imie', '$dane_nazwisko', '$dany_email', '$dane_haslo', 'obsługa')");
...
<body>
	...
	<div id="bigBox">
		...
		<div id="smallBox">
			...
			<form method="post">
				<div style="font-size: 30px;margin: 10px;color: white;">Rejestracja członka obsługi:</div>
	...
	<form action = "signup.php">
		<input type="submit" id="button2" value = "Rejestracja artystów" />
	</form>
	</div>
	</div>
	...
	<div id = "echoo">
		<?php
		if($echo1) {echo '<span style="color: white; font-size: 20px; font-weight: bold"> ' . "Proszę wprowadzić prawidłowe dane rejestracji!" . '</span>';}
		?>
	</div>
